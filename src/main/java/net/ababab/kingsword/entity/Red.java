...
public class Red extends ElementsKingswordMod.ModElement {
	public Red(ElementsKingswordMod instance) {
		super(instance, 90);
	}
				...
				for (int j = 0; j < 3; j++) {
					...
					for (int i1 = k; i1 >= l; i1--) {
						...
						for (int j1 = 0; j1 < 5; j1++) {
							...
							bufferbuilder.pos(d10 + d2, y + i1 * 16, d11 + d3).color(170.0F, 0.0F, 0.0F, 0.5F).endVertex();
							bufferbuilder.pos(d8 + d4, y + (i1 + 1) * 16, d9 + d5).color(170.0F, 0.0F, 0.0F, 0.5F).endVertex();}
						tessellator.draw();
					}
				}
			}
