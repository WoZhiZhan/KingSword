...
public class EntityColorLightningBlot extends ElementsKingswordMod.ModElement {
	public EntityColorLightningBlot(ElementsKingswordMod instance) {
		super(instance, 90);
	}
				...
				for (int j = 0; j < 3; j++)
				{
					...
					for (int i1 = k; i1 >= l; i1--)
					{
						...
						for (int j1 = 0; j1 < 5; j1++)
						{
							...
							bufferbuilder.pos(d10 + d2, i1 * 16 + y, d11 + d3).color(f, f1, f2, 0.5F).endVertex();
							bufferbuilder.pos(d8 + d4, (i1 + 1) * 16 + y, d9 + d5).color(f, f1, f2, 0.5F).endVertex();
						}
					tessellator.draw();
					}
				}
